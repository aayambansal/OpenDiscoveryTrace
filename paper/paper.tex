...
\begin{abstract}
Existing benchmarks for AI scientific agents evaluate final outputs but discard the reasoning process, making it impossible to audit methodology, diagnose failures, or distinguish luck from skill.
We propose \dataset, a structured dataset of 372 complete agent trajectories (124 tasks, 3 frontier models, 4 science domains) recorded as 9-field-per-step traces extending ReAct with phase tracking, error logging, revision triggers, and confidence.
Pilot analysis reveals that all three models achieve indistinguishable success rates (${\sim}69\%$, $p{=}0.997$) yet Claude Opus 4.6 produces $30{\times}$ more errors than GPT-5.4 ($2.5$ vs.\ $0.08$, $p{<}0.0001$, Cliff's $\delta{=}0.613$), predominantly tool misuse (66.7\%) versus reasoning errors (83.6\%), a dimension invisible to output-only evaluation.
We release the dataset, schema, harness, and five benchmark tasks under CC-BY-4.0.\footnote{Submitted to the AI for Science workshop (ICML 2026).}
\end{abstract}
]
...
\dataset comprises 372 complete trajectories spanning 124 tasks across four scientific domains (drug discovery, materials science, genomics, scientific literature) at three difficulty levels (easy, medium, hard). Three frontier models (GPT-5.4, Claude Opus 4.6, Gemini 3.1 Pro) each execute all 124 tasks, yielding a fully balanced design with 31 tasks per model per domain. The dataset additionally includes 30 open-source trajectories from Qwen2.5-1.5B-Instruct (enabling reproducible baselines without proprietary API access) and 30 live-retrieval variant trajectories for ecological validity.
...
\begin{figure}[t]
    ...
    \vspace{-1.5em}
    \caption{\footnotesize \textbf{\dataset architecture.} Three frontier models execute 124 tasks through a six-phase scientific workflow. Each step is logged as a 9-field trace. Failed steps trigger error-revision loops (dashed). The 372 trajectories form a structured corpus of AI scientific process traces.}
...
\textbf{Phase~1 (current).} 372 trajectories across 3 frontier models, 124 tasks, 4 domains. Schema, harness, and benchmark tasks complete. 30 open-source and 30 live-retrieval trajectories collected. Estimated cost: ${\sim}$\$300 API calls plus GPU cluster time. All infrastructure (harness, schema validator, analysis pipeline) is operational.

\textbf{Phase~2 (months 1--2).} Complete all 200 designed tasks across existing models. Add two open-source models (Llama~3.1, Mistral). Target: 1,000+ trajectories with 5 models, enabling robust cross-architecture comparisons and sufficient statistical power for domain-specific and difficulty-stratified analyses.
...
Models represented & 3 (+ 1 open-source) \\
Domains covered & 4 \\
